...
public class ApiTokenService {
    ...
    private TokenExchangeResponse? GetCachedApiToken(HttpContext ctx, ApiConfigurationRecord apiConfig) {
    ...
    private void SetCachedApiToken(HttpContext ctx, ApiConfigurationRecord apiConfig, TokenExchangeResponse response) {
        var cache = ctx.Session.GetObject<Dictionary<string, TokenExchangeResponse>>(SessionKeys.API_ACCESS_TOKEN);
        if (cache == null) {
    ...
    public async Task<string> LookupApiToken(HttpContext ctx, ApiConfigurationRecord apiConfig, string token)
    {
