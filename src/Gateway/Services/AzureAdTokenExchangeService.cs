...
public class AzureAdTokenExchangeService : ITokenExchangeService
{
    ...
    public AzureAdTokenExchangeService(GatewayConfigurationRecord config, DiscoveryDocument disco)
    {
        this.disco = disco;
        this.config = config;
    }

    public async Task<TokenExchangeResponse> Exchange(string accessToken, ApiConfigurationRecord apiConfig)
    {
