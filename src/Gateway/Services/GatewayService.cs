...
public class GatewayService
{
    ...
    private GatewayConfigurationRecord config;
    private ApiTokenService apiTokenService;
    private ILogger<GatewayService> logger;

    public GatewayService(
        TokenRefreshService tokenRefreshService,
        GatewayConfigurationRecord config,
    ...
    private async Task<string> GetApiToken(HttpContext ctx, ApiTokenService apiTokenService, string token, ApiConfigurationRecord? apiConfig)
    {
    ...
    public async Task AddToken(HttpContext ctx)
    {
        ...
        if (!string.IsNullOrEmpty(token) && apiConfig != null)
        {
            ...
            ctx.Request.Headers.Append("Authorization", "Bearer " + apiToken);
        }
    }
